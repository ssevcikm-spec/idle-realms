(function () {
  ...
  G.panelBase = function () {
    ...
    for (const bid in G.BASE_BUILDINGS) {
      ...
      const rateInfo = lvl > 0 ? `<div class="act-sub" style="color:#8fbf7a">Produkce: +${(def.rate(lvl)*3600).toFixed(1)}/h</div>` : '';
      html += `<div class="recipe-row ${maxed || !check.ok ? 'locked' : ''}">
        ...
        <button class="btn-sm" ${check.ok ? '' : 'disabled'} data-action="build-base" data-building="${bid}">
          ${maxed ? 'MAX' : 'Postavit'}
        </button>
      </div>`;
    }
