  ...
  function renderCombat(cb) {
    ...
    let html = `<div class="modal-backdrop"><div class="modal wide combat-modal">
      <div class="combat-head">
        <div class="combat-title">⚔️ Souboj — kolo ${round}</div>
      </div>`;
    ...
    for (const e of cb.enemy) {
      ...
      html += `<div class="combat-unit ${e.alive ? '' : 'dead'}">
    ...
    for (const a of cb.ally) {
      ...
      const hpColor = hpPct > 50 ? '#8fbf7a' : hpPct > 25 ? '#e0bb5e' : '#c05a45';
      const buffs = a.buffs && Object.keys(a.buffs).length ? '⬆️' : '';
      html += `<div class="combat-unit ${a.alive ? '' : 'dead'}">
        <div class="combat-unit-icon" style="background:${u ? u.color : '#555'};color:#fff">${u ? G.esc(u.name.charAt(0)) : '?'}</div>
        <div class="combat-unit-name">${u ? G.esc(u.name.split(' ')[0]) : '?'} ${buffs}</div>
        <div class="progress"><div class="progress-bar" style="width:${hpPct}%;background:${hpColor}"></div></div>
        ...
        for (const a of abilitiesByAlly) {
          ...
          for (const aid of a.abilities) {
            ...
            html += `<button class="cb-ability ${queued ? 'queued' : ''} ${disabled ? 'disabled' : ''}"
              ...
              ${disabled ? 'disabled' : ''}
              title="${G.esc(ab.desc)} • CD ${ab.cooldown} • ${ab.stamina}⚡">
              ${ab.icon} ${G.esc(ab.name)}${cd > 0 ? ` (${cd})` : ''}
            </button>`;
          }
          html += `</div></div>`;
        }
    ...
    if (!cb.finished) {
      html += `<div class="combat-actions">
        ...
        <button class="btn ghost" data-action="combat-auto">⚡ Automaticky (10 kol)</button>
      </div>`;
    } else {
