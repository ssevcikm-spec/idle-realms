(function () {
  ...
  /** Zajistí, že každá frakce má naplánované volby. */
    ...
    for (const fid in G.FACTIONS) {
      if (!G.state.politics.factions[fid]) {
        G.state.politics.factions[fid] = {
          ...
          phase: 'idle',           // idle | election
          phaseEndsAt: 0,
          candidates: [],
          winner: null,
          playerSupport: 0,
          playerCandidateId: null,
          history: []
        };
      }
    }
  ...
  /** Naplánuje nové volby pro frakci. */
  function startElection(fid) {
    ...
    for (let i = 0; i < candidateCount; i++) {
      ...
      candidates.push({
        ...
        name,
        program: progId,
        support: G.randInt(1, 5)      // výchozí podpora od NPC
      });
    }
    ...
    if (G.getRep(fid) >= G.POLITICS.candidacyReputation) {
      ...
      candidates.push({
        id: st.playerCandidateId,
        name: G.state.playerName || 'Ty',
        program: 'traditional',
        support: 0,
        isPlayer: true
      });
    } else {
      st.playerCandidateId = null;
    }
  ...
  G.tickPolitics = function (dt) {
    ...
    for (const fid in G.state.politics.factions) {
      const st = G.state.politics.factions[fid];
      if (st.phase === 'idle' && G.state.time >= st.nextElection) {
        startElection(fid);
      } else if (st.phase === 'election' && G.state.time >= st.phaseEndsAt) {
        resolveElection(fid);
      }
    }
  };

  /** Hráč podpoří kandidáta. */
  ...
  G.politicsOverview = function () {
    ...
    for (const fid in G.FACTIONS) {
      ...
      out.push({
        faction: f,
        phase: st.phase,
        nextElection: st.nextElection,
        phaseEndsAt: st.phaseEndsAt,
        candidates: st.candidates,
        winner: st.winner,
        playerSupport: st.playerSupport || 0
      });
    }
    return out;
  };

  /** Aplikuje politické modifikátory na cenu. */
  ...
  /** Bonus k XP z politiky (traditional). */
