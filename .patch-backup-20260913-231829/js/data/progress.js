(function () {
  ...
  G.QUEST_TEMPLATES = {
    ...
    deliver_ingot:  { id:'deliver_ingot', kind:'deliver', weight:6, minSize:2, generate:(s)=>({ text:`Zbrojíř potřebuje ${2+s}× železný ingot.`, need:[{material:'iron_ingot',qty:2+s}], reward:{ gold:Math.round(90*(2+s)*(1+s*0.15)), renown:4, rep:12 }, deadline:1800+s*300 }) },
  ...
  function anyUnitSkill(s, skillId, lvl) { for (const u of s.units) { const sk = u.skills[skillId]; if (sk && sk.level >= lvl) return true; } return false; }
  function countFactionsAbove(s, threshold) { let n = 0; const rep = s.reputation || {}; for (const fid in rep) if (rep[fid] >= threshold) n++; return n; }

  G.STORY_QUESTS = [
    { id:'arrival', title:'Neznámý poutník', text:'Sedíš u ohně. Starý muž s holí se přitočí. „Tak ty jsi ten nový vůdce. Máš plán?"', trigger:(s)=>s.time >= 120,
      choices:[
        { text:'Postavit vlastní osadu.', effects:[ { type:'log', text:'🏕️ Rozhodl ses vybudovat vlastní základnu (při renomé 25).' }, { type:'set_flag', flag:'plan', value:'base' }, { type:'renown', value:3 } ] },
        { text:'Ovládnout obchod.', effects:[ { type:'log', text:'⚖️ Rozhodl ses ovládnout obchodní stezky.' }, { type:'set_flag', flag:'plan', value:'trade' }, { type:'bonus_rep', faction:'league', value:5 } ] },
        { text:'Stát se legendou v boji.', effects:[ { type:'log', text:'⚔️ Tvé jméno bude znít v každém hostinci.' }, { type:'set_flag', flag:'plan', value:'war' }, { type:'renown', value:2 } ] }
      ] },
    { id:'mountain_message', title:'Zpráva z hor', text:'Posel přináší zprávu: „V dolech na severu se ztratili dva horníci."', trigger:(s)=>s.time >= 600 && anyUnitSkill(s, 'mining', 3),
      choices:[
        { text:'Vyslat skupinu hledat.', effects:[ { type:'cost_mat', material:'bread', qty:2 }, { type:'mat', material:'iron_ore', qty:8 }, { type:'renown', value:4 }, { type:'bonus_rep', faction:'guild', value:8 }, { type:'log', text:'⛏️ Našli jste oba horníky i bohatou žílu.' } ] },
        { text:'To není náš problém.', effects:[ { type:'bonus_rep', faction:'guild', value:-5 }, { type:'log', text:'😐 Hornický cech si to zapamatuje.' } ] }
      ] },
    { id:'forest_call', title:'Volání lesa', text:'Z temnoty vystoupí muž v kožešinách. „Jsem z Lesního bratrstva. Přítel, nebo cizinec?"', trigger:(s)=>G.getRep('brotherhood') >= 10,
      choices:[
        { text:'Přísahat lesu.', effects:[ { type:'set_flag', flag:'oath', value:'forest' }, { type:'bonus_rep', faction:'brotherhood', value:15 }, { type:'bonus_rep', faction:'guild', value:-8 }, { type:'log', text:'🌲 Složil jsi přísahu Lesnímu bratrstvu.' } ] },
        { text:'Zůstat neutrální.', effects:[ { type:'log', text:'🤝 Bratrstvo odchází.' } ] }
      ] },
    { id:'merchant_call', title:'Kupecká výzva', text:'Přijde k tobě bohatý kupec: „Chceš-li prosperovat, musíš obchodovat. Pošli někoho, kdo zná cesty."', trigger:(s)=>s.units.length >= 4 && s.resources.gold >= 200,
      choices:[
        { text:'Vyšlu obchodníka.', effects:[ { type:'log', text:'🐎 Rozhodl ses vyslat obchodníka na cesty.' }, { type:'set_flag', flag:'merchant_unlocked', value:true }, { type:'bonus_rep', faction:'league', value:8 } ] },
        { text:'Zatím ne.', effects:[ { type:'log', text:'Kupec odchází s úsměvem.' } ] }
      ] },
    { id:'cave_shadows', title:'Stíny v jeskyni', text:'Tvá nejzkušenější postava se vrací bledá. „Není tam jen krystal."', trigger:(s)=>anyUnitSkill(s, 'combat', 5),
      choices:[
        { text:'Vyčistit jeskyni.', effects:[ { type:'combat', difficulty:6 }, { type:'mat', material:'crystal', qty:3, quality:'fine' }, { type:'renown', value:8 }, { type:'set_flag', flag:'cave_cleared', value:true }, { type:'log', text:'⚔️ Jeskyně je čistá.' } ] },
        { text:'Zapečetit vchod.', effects:[ { type:'log', text:'🪨 Zapečetili jste jeskyni.' }, { type:'set_flag', flag:'cave_sealed', value:true } ] }
      ] },
    { id:'council', title:'Rada starších', text:'Zástupci frakcí se sešli, aby rozhodli o tvém místě ve světě.', trigger:(s)=>countFactionsAbove(s, 30) >= 2,
      choices:[
        { text:'Zůstat věrný Koruně.', effects:[ { type:'bonus_rep', faction:'crown', value:20 }, { type:'bonus_rep', faction:'brotherhood', value:-15 }, { type:'set_flag', flag:'allegiance', value:'crown' }, { type:'log', text:'👑 Přísahal jsi Koruně.' } ] },
        { text:'Stát na straně svobodných.', effects:[ { type:'bonus_rep', faction:'brotherhood', value:20 }, { type:'bonus_rep', faction:'guild', value:10 }, { type:'bonus_rep', faction:'crown', value:-20 }, { type:'set_flag', flag:'allegiance', value:'free' }, { type:'log', text:'🌲 Přidal ses k svobodným.' } ] },
        { text:'Nezávislost.', effects:[ { type:'renown', value:15 }, { type:'set_flag', flag:'allegiance', value:'independent' }, { type:'log', text:'⚖️ Zůstal jsi nezávislý.' } ] }
      ] },
    { id:'new_beginning', title:'Nový začátek', text:'Tvá první kapitola je u konce. Před tebou se otevírá nová cesta.', trigger:(s)=>(s.prestige && s.prestige.level >= 1) || s.resources.renown >= 100,
      choices:[
        { text:'Pokračovat dál.', effects:[ { type:'renown', value:20 }, { type:'log', text:'🌟 Kapitola jedna skončila.' }, { type:'set_flag', flag:'chapter1_done', value:true } ] }
      ] }
  ];
  G.STORY_CHECK_INTERVAL = 10;

  function anySkillLevel(s, lvl) { for (const u of s.units) for (const sid in u.skills) if (u.skills[sid].level >= lvl) return true; return false; }
  function anyBaseBuilding(s) { if (!s.base || !s.base.buildings) return false; for (const bid in s.base.buildings) if (s.base.buildings[bid] > 0) return true; return false; }
  function allBaseBuildingsAt(s, lvl) { if (!s.base || !s.base.buildings) return false; let n = 0; for (const bid in G.BASE_BUILDINGS) if ((s.base.buildings[bid] || 0) >= lvl) n++; return n === Object.keys(G.BASE_BUILDINGS).length; }
  function anyFactionAbove(s, threshold) { const rep = s.reputation || {}; for (const fid in rep) if (rep[fid] >= threshold) return true; return false; }
  function totalSettlementBuildings(s) { let n = 0; if (!s.buildings) return 0; for (const sid in s.buildings) for (const bid in s.buildings[sid]) n += s.buildings[sid][bid] || 0; return n; }
  function anySettlementRepAbove(s, threshold) { const r = s.settlementRep || {}; for (const sid in r) if (r[sid] >= threshold) return true; return false; }
  function anyAmbitionDone(s, n) { let cnt = 0; for (const u of s.units) { for (const a of (u.ambitions || [])) if (a.done) cnt++; } return cnt >= n; }
  ...
  G.ACHIEVEMENTS = [
    ...
    { id:'merchant_100', name:'Karavanní král', icon:'👑', desc:'Proveď 100 obchodníkových cest.', reward:{gold:1500,renown:30}, check:(s)=>(s.stats.merchantTrades||0)>=100 },
    ...
    { id:'politician', name:'Politik', icon:'🏛️', desc:'Vyhraj volby ve frakci.', reward:{gold:300,renown:10}, check:(s)=>(s.stats.electionsWon||0)>=1 },
    { id:'dynasty_3', name:'Rodokmen', icon:'🌳', desc:'Dosáhni 3. generace dynastie.', reward:{gold:400,renown:10}, check:(s)=>(s.dynasty && s.dynasty.generations)>=3 },
