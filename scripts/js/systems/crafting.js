(function () {
  ...
  G.craft = function (recipeId) {
    ...
    if (crafter && G.perkEffects) {
      ...
      for (const m in eff.craftingSave) saved[m] = (saved[m] || 0) + eff.craftingSave[m];
    }
    if (crafter && G.synergyBonus) {
      const synSave = G.synergyBonus(crafter, 'craftSave');
      for (const m in synSave) saved[m] = (saved[m] || 0) + synSave[m];
    }
    ...
    let q;
    if (crafter) {
      q = G.rollQualityWithBonus(bestLvl, [crafter], r.skill, synQuality);
    } else {
      q = G.rollQuality(bestLvl, [], r.skill);
    }
    ...
    if (q === 'masterwork' && crafter && G.chance(0.30)) {
      ...
      G.state.masterworks.push({
        id: 'mw' + Date.now(), name: itemName, recipe: rid2name(recipeId),
        craftsman: crafter.name, craftsmanId: crafter.id,
        material: r.output.material, qty: batch, time: G.state.time
      });
      extra = ' ✨';
    } else {
