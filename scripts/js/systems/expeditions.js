(function () {
  ...
  function foodNeededFor(days, partySize) {
    return Math.ceil(days * partySize * G.EXPEDITION_FOOD_PER_UNIT_PER_DAY);
  }
  function availableFood() {
    return G.matCount('bread') + G.matCount('fish');
  }
  function consumeFood(amount) {
    let remaining = amount;
    for (const mat of ['bread', 'fish']) {
      if (remaining <= 0) break;
      const have = G.matCount(mat);
      const take = Math.min(have, remaining);
      if (take > 0) { G.matRemove(mat, take); remaining -= take; }
    }
    return remaining === 0;
  }
  ...
  G.startExpedition = function (expeditionId, unitIds) {
    ...
    if (availableFood() < foodCost) {
      return { ok:false, reason:`Potřebuješ ${foodCost}× jídlo na cestu (chléb/ryba), máš ${availableFood()}.` };
    }
    ...
    const exp = {
      ...
      foodCost: foodCost, outcome: null
    };
